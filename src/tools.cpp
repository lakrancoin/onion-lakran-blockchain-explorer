...
{
    pair<uint64_t, uint64_t> sum_lak {0, 0};
    ...
    {
        sum_lak.first += vout["amount"].get<uint64_t>();
        ++sum_lak.second;
    }
...
{
    pair<uint64_t, uint64_t> sum_lak {0ULL, 0ULL};
        ...
        vector<txin_to_key>& input_key_imgs)
{
    ...
    uint64_t lak_outputs       {0};
    uint64_t lak_inputs        {0};
    uint64_t mixin_no          {0};
    ...
    for (size_t i = 0; i < input_no; ++i)
    {
        ...
        lak_inputs += tx_in_to_key.amount;
    ...
    return {lak_outputs, lak_inputs, mixin_no, num_nonrct_inputs};
};
...
summary_of_in_out_rct(const json& _json)
{
    ...
    for (const json& vout: _json["vout"])
    {
        lak_outputs += vout["amount"].get<uint64_t>();
    }
    ...
    return {lak_outputs, lak_inputs, no_outputs, no_inputs, mixin_no, num_nonrct_inputs};
};
...
sum_money_in_inputs(const string& json_str)
{
    ...
    for (json& vin: j["vin"])
    {
        sum_lak.first += vin["key"]["amount"].get<uint64_t>();
        ++sum_lak.second;
    }
...
{
    array<uint64_t, 2> sum_lak;

    sum_lak[0] = sum_money_in_inputs(tx);
    sum_lak[1] = sum_money_in_outputs(tx);
