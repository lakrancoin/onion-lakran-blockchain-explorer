...
static
string
lak_amount_to_str(const uint64_t& lak_amount,
                  string _format="{:0.9f}",
                  bool zero_to_question_mark=true)
{
    ...
    {
        amount_str = fmt::format(_format, LAK_AMOUNT(lak_amount));
    }
    else
    {
        if (lak_amount > 0 && zero_to_question_mark == true)
        {
            amount_str = fmt::format(_format, LAK_AMOUNT(lak_amount));
        }
    }
